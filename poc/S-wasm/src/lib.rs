...
pub fn render_aurora(
    ...
    apply_grain(t, grain, width, height, &mut data);
    data
}
...
fn blob_state(
    ...
    let radius = min_dim * blob.size_base * breath.max(0.0);
        ...
        for x in (0..width).step_by(step) {
            ...
            for sy in y..(y + step).min(height) {
                for sx in x..(x + step).min(width) {
                    ...
                    let blended = [
                        mix(current[0], overlay_channel(current[0], noise), grain),
                        mix(current[1], overlay_channel(current[1], noise), grain),
                        mix(current[2], overlay_channel(current[2], noise), grain),
                    ];

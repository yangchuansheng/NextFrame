...
export const meta = {
  ...
  params: {
    text: { type: "string", required: true, label: "标题文字", semantic: "supports inline HTML like <span style='color:#da7756'>重点</span>", group: "content" },
...
export function render(t, params, vp) {
  ...
  var fs = p.fontSize || 42;
  var color = p.color || "#f5ece0";
  var posStyle = p.y > 0
    ? "position:absolute;left:60px;right:60px;top:" + p.y + "px;text-align:center"
    : "position:absolute;inset:0;display:flex;align-items:center;justify-content:center;padding:0 60px";
  return '<div style="' + posStyle + '">' +
    '<div style="font:700 ' + fs + 'px Georgia,\'Noto Serif SC\',serif;color:' + color + ';text-align:center;line-height:1.3;opacity:' + op + '">' + (p.text || '') + '</div>' +
  '</div>';
}

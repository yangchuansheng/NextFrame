...
export async function run(argv) {
  ...
  if (!projectName || !episodeName) {
    emit({ ok: false, error: { code: "USAGE", message: "usage: nextframe audio-get <project> <episode> [--segment=N] [--root=PATH] [--json]" } }, flags);
    return 3;
  }
  ...
  const value = segmentNumber === undefined
    ? pipeline.audio
    : pipeline.audio.segments.find((segment) => Number(segment.segment) === segmentNumber) || null;
